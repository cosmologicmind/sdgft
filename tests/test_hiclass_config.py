...
from sdgft.hiclass_config import hiclass_params, cobaya_params, hiclass_ini_string, BETA_ISO_F
...
class TestIsocurvature:
    """Verifies that the isocurvature initial conditions are properly set and locked."""

    def test_hiclass_isocurvature_initialization(self):
        """Verifies that the NID-Anfangsbedingungen standardmäßig korrekt gesetzt sind."""
        params = hiclass_params()
        
        assert params["ic"] == "ad,nid"
        assert params["f_nid"] == pytest.approx(1.0 / 36.0, rel=1e-15)

    def test_cobaya_parameter_locking(self):
        """Stellt sicher, dass f_nid in den Cobaya-MCMC-Parametern starr eingefroren ist."""
        config = cobaya_params()
        params_dict = config["params"]
        
        assert "f_nid" in params_dict
        assert "value" in params_dict["f_nid"]
        assert params_dict["f_nid"]["value"] == BETA_ISO_F
        
        assert "ic" in params_dict
        assert params_dict["ic"]["value"] == "ad,nid"

    def test_ini_string_propagation(self):
        """Überprüft, ob die Isokurvatur-Parameter sauber in den .ini-String exportiert werden."""
        ini_str = hiclass_ini_string()
        
        assert "ic = ad,nid" in ini_str
        assert f"f_nid = {BETA_ISO_F}" in ini_str
