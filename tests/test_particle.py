...
class TestNeutrinoMixing:
    ...
    theta_12 uses the cross-sector relation: sin^2(theta_12) = 1/3 - beta_iso = 11/36
    = 33.56 deg (0.2 sigma from observed 33.41).
    """

    def test_theta_12_formula_correct(self):
        """arcsin(sqrt(1/3 - 16*delta_g^2)) in degrees."""
    ...
    def test_theta_12_value(self):
        """theta_12 ~ 33.56 deg."""
        assert abs(THETA_12 - 33.56) < 0.01

    def test_theta_12_close_to_observed(self):
        """Observed: 33.41 +/- 0.75 deg."""
    ...
    def test_theta_12_tbm_limit(self):
        ...
        val = theta_12(0.0)
        expected = math.degrees(math.asin(math.sqrt(1.0 / 3.0)))
        assert abs(val - expected) < 1e-12

    def test_theta_23_formula_correct(self):
        """arcsin(sqrt(1/2 + Delta/3)) in degrees."""
    ...
    def test_theta_23_actual_value(self):
        """Formula yields ~48.99°."""
        assert abs(THETA_23 - 48.99) < 0.01
