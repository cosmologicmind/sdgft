...
MZ = 91.1876  # Z-Masse in GeV

def get_lambda_cabibbo() -> float:
    """Berechnet den Cabibbo-Expansionsparameter lambda aus dem Quantenverhältnis."""
    return math.sqrt(float(DELTA_G) / float(DELTA)) * (1.0 - float(DELTA_G))

def get_jarlskog_topological() -> float:
    """Berechnet die exakte topologische Jarlskog-Invariante J_CP."""
    return (float(DELTA)**2 * float(DELTA_G)**2) / math.sqrt(6.0)

def get_higgs_mass(conformal: bool = True) -> float:
    """
    Berechnet die Higgs-Masse aus der konformen 6-Kegel-Geometrie.
    Standard-GR liefert ~110.84 GeV, die konforme Skalierung liefert ~125.11 GeV.
    """
    raw_geometric = MZ * math.sqrt((23.0 / 12.0) * (37.0 / 48.0))
    if not conformal:
        return raw_geometric
    
    # Konformer Skalierungsfaktor gekoppelt an die F4-Topologie (Delta / phi)
    conformal_factor = 1.0 + (float(DELTA) / PHI)
    return raw_geometric * conformal_factor


def get_yukawa_textures() -> np.ndarray:
    """Liefert die fundamentale diagonale Yukawa-Hierarchie-Matrix."""
    d = float(DELTA_G)
    D = float(DELTA)
    return np.diag([d**2, D**2, (1.0 - D)**2])

def get_flavor_rotation_matrices() -> tuple[np.ndarray, np.ndarray]:
    """
    Generiert die separaten unitären Rotationsmatrizen V_u und V_d
    basierend auf dem Cabibbo-Winkel lambda.
    """
    lam = get_lambda_cabibbo()
    
    # Beispielhafte starr fixierte projektive Drehung für den Up- und Down-Sektor
    theta_u = lam * 0.25
    theta_d = lam * 0.75
    
    V_u = np.array([
        [math.cos(theta_u), -math.sin(theta_u), 0.0],
        [math.sin(theta_u),  math.cos(theta_u), 0.0],
        [0.0,                0.0,               1.0]
    ])
    
    V_d = np.array([
        [math.cos(theta_d), -math.sin(theta_d), 0.0],
        [math.sin(theta_d),  math.cos(theta_d), 0.0],
        [0.0,                0.0,               1.0]
    ])
    
    return V_u, V_d

def compute_v_ckm() -> np.ndarray:
    """Berechnet die CKM-Matrix aus dem Produkt V_u^dagger * V_d."""
    V_u, V_d = get_flavor_rotation_matrices()
    return np.dot(V_u.T, V_d)
