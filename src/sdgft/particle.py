...
def theta_12(delta_g: float) -> float:
    """Solar mixing angle: theta_12 = arctan(1/sqrt(2)) * (1 - delta_g).

    TBM base value corrected by single-vertex sampling: the observer
    occupies 1 of 24 equivalent 24-cell vertices, reducing the effective
    solar mixing to (N-1)/N = 23/24 of TBM.
    ...
    return math.degrees(tbm * (1.0 - delta_g))


def theta_23(delta: float) -> float:
    """Atmospheric mixing angle: theta_23 = 45*(1 + Delta/sqrt(6)).
...
def theta_13(delta: float) -> float:
    """Reactor mixing angle: theta_13 = arcsin(Delta/sqrt(2)).
...
"""Solar mixing angle: TBM * (1 - 1/24) ~ 33.80 deg."""

THETA_23: float = theta_23(DELTA_F)
"""Atmospheric mixing angle ~ 47.6 deg (note: fixed monograph has 47.6)."""

THETA_13: float = theta_13(DELTA_F)
"""Reactor mixing angle ~ 8.47 deg."""
